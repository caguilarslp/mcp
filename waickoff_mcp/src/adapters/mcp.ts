    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        switch (name) {
          ...
          // Trap Detection Tools (TASK-012)
          case 'detect_bull_trap':
            result = await this.handlers.handleDetectBullTrap(args);
            break;
          case 'detect_bear_trap':
            result = await this.handlers.handleDetectBearTrap(args);
            break;
          case 'get_trap_history':
            result = await this.handlers.handleGetTrapHistory(args);
            break;
          case 'get_trap_statistics':
            result = await this.handlers.handleGetTrapStatistics(args);
            break;
          case 'configure_trap_detection':
            result = await this.handlers.handleConfigureTrapDetection(args);
            break;
          case 'validate_breakout':
            result = await this.handlers.handleValidateBreakout(args);
            break;
          case 'get_trap_performance':
            result = await this.handlers.handleGetTrapPerformance(args);
            break;
