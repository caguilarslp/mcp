"""
Advanced Data Sanitizer for LLM Service
Handles PII detection, data cleaning, and security compliance
"""
...
class DataSanitizer:
    ...
    def _build_malicious_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "excessive_special": re.compile(r'[!@#$%^&*()_+\-=\[\]{};\':"\\|,.<>\/?]{20,}')
        }
    
    async def sanitize_request_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for key, value in data.items():
                if isinstance(value, str):
                    sanitized[key] = await self._sanitize_string(value, key)
                elif isinstance(value, dict):
                    sanitized[key] = await self.sanitize_request_data(value)
                elif isinstance(value, list):
                    sanitized[key] = await self._sanitize_list(value)
                else:
                    sanitized[key] = value
    ...
    async def _sanitize_string(self, text: str, context: str = "") -> str:
        """
        ...
        try:
            ...
            if context.lower() in ['message', 'query', 'prompt']:
                sanitized = await self._remove_pii(sanitized)
            
            # 3. Remove malicious patterns
            sanitized = await self._remove_malicious_content(sanitized)
    ...
    async def _sanitize_list(self, items: List[Any]) -> List[Any]:
        ...
        for item in items:
            if isinstance(item, str):
                sanitized.append(await self._sanitize_string(item))
            elif isinstance(item, dict):
                sanitized.append(await self.sanitize_request_data(item))
    ...
    async def _remove_malicious_content(self, text: str) -> str:
        ...
        if threats_detected:
            logger.warning(f"Malicious content detected and removed: {', '.join(threats_detected)}")
    ...
    def validate_clean_data(self, original: str, sanitized: str) -> Dict[str, Any]:
        ...
        try:
            ...
            excessive_reduction = length_reduction > 0.5
            
            # Check if critical content might have been removed
            critical_keywords = ["analyze", "symbol", "price", "technical", "market", "trading"]
            keywords_preserved = sum(1 for keyword in critical_keywords if keyword.lower() in sanitized.lower())
            keywords_total = sum(1 for keyword in critical_keywords if keyword.lower() in original.lower())
            ...
            return {
                ...
                "is_valid": not excessive_reduction and keyword_preservation > 0.7
            }
    ...
    async def sanitize_for_logging(self, data: Any, max_length: int = 200) -> str:
        """
        ...
        try:
            ...
            # Remove PII
            sanitized = await self._remove_pii(text)
